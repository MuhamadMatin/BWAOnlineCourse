...
class StudentAnswerController extends Controller
{
    ...
    {
        $question_detail = CourseQuestion::where('id', $question)
            ->first();
        ...
        try {
            ...
            if ($selectedAnswer->course_question_id != $question) {
                $error = ValidationException::withMessages([
                    'system_error' => ['System Error!' . 'Answer not found'],
                ]);
            ...
            if ($existingAnswer) {
                $error = ValidationException::withMessages([
                    'system_error' => ['System Error!' . 'You already answered'],
                ]);
            ...
            if ($nextQuestion) {
                return redirect()->route('dashboard.learing.course', ['course' => $course->id, 'question' => $nextQuestion->id]);
            } else {
                return redirect()->route('dashboard.learing.finished.course', $course->id);
            }
        } catch (\Exception $e) {
            DB::rollBack();
            $error = ValidationException::withMessages([
